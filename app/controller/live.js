...
class BuildController extends Controller {
  ...
  async add() {
    ...
    let { roomId,checkInDate,promiseFee,monthFee,dianFee,waterFee,mangeFee,otherFee,payDay,howlong } = ctx.request.body; // 得到post请求的body参数
    ...
    try {
      await ctx.model.Live.create({
        roomId,checkInDate,promiseFee,monthFee,dianFee,waterFee,mangeFee,otherFee,payDay,howlong,uid
      });
  ...
  async liveInfo() {
    ...
    try {
      const one = await ctx.model.Live.findByPk(buildId);
  ...
  async queryBuild() {
    const { ctx, app } = this;
    let { page, limit, uid, checkInDate, status } = ctx.request.body;
